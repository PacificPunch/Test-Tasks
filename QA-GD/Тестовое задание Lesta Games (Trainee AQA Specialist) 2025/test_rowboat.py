...
def test_zero_max_speed()  # проверка создания лодки c нулевой скоростью
    with pytest.raises(AssertionError)
        Rowboat(0, 3)  # Создание лодки с нулевой максимальной скоростью должно вызвать ошибку

def test_zero_max_passengers()  # проверка создания лодки с нулевым колл макс пассажиров
    with pytest.raises(AssertionError)
        Rowboat(5, 0)  # Создание лодки с нулевым количеством пассажиров должно вызвать ошибку
#######################################################################################################
...
def test_passenger_limit()
    rowboat = Rowboat(5, 3)
    for i in range(3)
        rowboat.add_passenger(fPassenger{i})
